...
export async function findByCharacter(character) {
  return Character.findOne({ character }).lean();
}

export async function findByCharacterWithFields(character, fields) {
  return Character.findOne({ character }, fields).lean();
}

export async function findBySemanticComponent(
  semanticComponent,
  excludeChar,
  limit = 100,
) {
...
export async function findByPhoneticComponent(
  phoneticComponent,
  excludeChar,
  limit = 100,
) {
...
export async function findByComponent(component, limit = 50) {
  return Character.find(
...
export async function searchByPinyinOrDefinition(query, limit = 50) {
  return Character.find(
    {
      $or: [
...
export async function findExactMatch(character) {
